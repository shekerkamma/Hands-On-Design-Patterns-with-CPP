...
// te = type erasure
// Only based on T not Deleter. Deleter is inside of the class.
template <typename T>
class smartptr_te {
    struct deleter_base {
...
public:
    // The constructor also uses the Deleter type:
    template <typename Deleter>
...
private:
    T* p_;
    // We point to the generic base type, not the Deleter type.
    // The Deleter type is lost, you could also say hidden or erased.
    deleter_base* d_;
};

// lb = local buffer (optimization)
template <typename T>
